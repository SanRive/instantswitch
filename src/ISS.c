 ...
 *   - removed the macOS 27 gesture sign inversion in iss_post_dock_swipe()
 *   - added ~10ms pacing between gesture phases (backport of upstream PR #88)
...
static bool iss_post_dock_swipe(CGSGesturePhase phase, ISSDirection direction, double velocity) {
    ...
    // NOTE: upstream inverts the sign here, but on this machine (macOS 27.0
    // 26A5388g) that makes the gesture move opposite to ISS's own internal
    // direction model, so iss_should_block_switch() guards the wrong edge.
    // Keeping the sign consistent with the legacy path makes them agree.
...
static bool iss_perform_switch_gesture(ISSDirection direction, double velocity) {
    ...
    // macOS 27 drops dock-swipe phases posted back-to-back, which shows up as
    // double moves (+/-2). Pace them ~10ms apart. (InstantSpaceSwitcher PR #88)
    const useconds_t phaseDelay = iss_requires_event_augmentation() ? 10000 : 0;
    if (!iss_post_dock_swipe(kCGSGesturePhaseBegan, direction, velocity)) return false;
    if (phaseDelay) usleep(phaseDelay);
    if (!iss_post_dock_swipe(kCGSGesturePhaseChanged, direction, velocity)) return false;
    if (phaseDelay) usleep(phaseDelay);
    if (!iss_post_dock_swipe(kCGSGesturePhaseEnded, direction, velocity)) return false;
    return true;
}
