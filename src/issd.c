...
static void onStdin(CFFileDescriptorRef f, CFOptionFlags cbTypes, void *info) {
    ...
    if (strcmp(buf, "left") == 0)  { predictions_refresh_if_stale(); printf("%d\n", iss_switch(ISSDirectionLeft)); }
    else if (strcmp(buf, "right") == 0) { predictions_refresh_if_stale(); printf("%d\n", iss_switch(ISSDirectionRight)); }
    else if (strcmp(buf, "reset") == 0) { predictions_force_reset(); printf("reset\n"); }
